...
> **ログイン画面が表示された場合**
>
> `grafana/otel-lgtm` イメージはデフォルトで匿名アクセス（Admin 権限）が有効なため、通常はログイン不要です。ログイン画面が表示された場合は `admin` / `admin` でサインインしてください。Grafana が完全に起動しきっていない直後にアクセスすると表示されることがあります。
...
データソース「Tempo」を選択し、「Search」タブを開きます。「Service Name」に `backend` を入力して「Run query」を押すと、バックエンドのスパンのみを絞り込めます。

### メトリクス（Prometheus）

データソース「Prometheus」を選択し、クエリフィールドに `todo_created_total` を入力して「Run query」を押すと、Todo 作成の累計回数が表示されます。`rate(todo_created_total[5m])` に変えると単位時間あたりの作成数を確認できます。

### ログ（Loki）

データソース「Loki」を選択し、クエリフィールドに `{service_name="frontend"}` を入力して「Run query」を押すと、フロントエンドのログが時系列で表示されます。ログ行をクリックすると「Tempo でトレースを表示」リンクが表示され、ログから対応するトレースに直接ジャンプできます。
